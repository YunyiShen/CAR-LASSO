...
void update_(int *n,         int *ilow,          int *ihigh,          int *ipt,       double *scum, 
  ...
  *huzmax = huz[j];
  ...
  while (ipt[i__] != 0) {
    if (control_count > *n) return;//error("Trap in ARS: infinite while in update_ of ars.cpp near l. 810\n");   /* added by AK */
             ...
             int k, int p, int n_sample)
  {
    ...
    *ifault = 0;
    ...
    while (! sampld && attempts < max_attempt) {
      ...
      alhu = hpx[i__] * (Z_curr(l,w) - x[i__]) + hx[i__] - *huzmax;
      if (Z_curr(l,w) > x[*ilow] && Z_curr(l,w) < x[*ihigh]) {
        /* compute alhl: value of the lower hull at point u1 */
        if (Z_curr(l,w) > x[i__]) {
          j = i__;
          i__ = ipt[i__];
        }
        alhl = hx[i__] + (Z_curr(l,w) - x[i__]) * (hx[i__] - hx[j]) / (x[i__] -  x[j]) - *huzmax;
        /* squeezing test */
        if (alhl - alhu > alu1) {
          sampld = true;
        }
      ...
      if (! sampld) {
        ...
        h_hprime = logPostZij_helper_Cpp(l, w,// which node
                                         Z_curr,
                                         mu_Z,Sigma_Z,// this is Sigma (cov) not Omega (percision)
                                         y,k,p,n_sample);
      ...
      attempts++;
        ...
        PutRNGstate();
    if (attempts >= max_attempt) return;//error ("Trap in ARS: Maximum number of attempts reached by routine spl1_\n"); 
    return;
        ...
        int k, int p, int n_sample)
{
  ...
  spl1_(&ns,             &iwv[4],        &iwv[1],      &iwv[2],       &iwv[iipt + 1], 
        ...
        l, w,// which node
        Z_curr,mu_Z,Sigma_Z,// this is Sigma (cov) not Omega (percision)
        y,k,p,n_sample);
  return;
    ...
    for(int j = 0 ; j < k ; ++j){
      for(int ww = 0 ; ww < m ; ++ww){
        ...
        h_hprime_temp = logPostZij_helper_Cpp(i,j,// which node
                                               ...
                                               y,k,p,n);
        hx[ww] = h_hprime_temp(0);
        hpx[ww] = h_hprime_temp(1);
      } //initial support
