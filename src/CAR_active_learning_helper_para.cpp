...
// det for full FI
struct full_det_worker : public Worker{
    // inputs
    const arma::mat & Omega_mcmc;
    const arma::mat & beta_mcmc;
    const arma::mat & mu_mcmc;
    ...
    // initialize
    full_det_worker(const arma::mat & Omega_mcmc,
                 const arma::mat & beta_mcmc,
                 const arma::mat & mu_mcmc,
                 const arma::mat & new_design,
                 const arma::mat & old_design,
...
// [[Rcpp::export]]
arma::vec full_det_para(const arma::mat & new_design,
                     const arma::mat & old_design,
                     const Rcpp::List & CAR_model,
                     int k, int p, int n_new, int n_old){
    const arma::mat & Omega_mcmc = CAR_model["Omega"];
    ...
    int n_mcmc = Omega_mcmc.n_rows;
    arma::vec log_dets(n_mcmc);
    full_det_worker full_det(Omega_mcmc, beta_mcmc,mu_mcmc,
                       new_design,old_design,k, p, 
                       n_new, n_old,
                       log_dets);
    
    parallelFor(0,n_mcmc,full_det);
    return(log_dets);
}

// [[Rcpp::export]]
double expected_full_det_para(const arma::vec & new_design,
                           const arma::mat & old_design,
                           const Rcpp::List & CAR_model,
                           int k, int p, int n_new, int n_old){
    arma::mat new_design_mat = arma::reshape(new_design,n_new,p);
    ...
    return(mean(
            full_det_para(new_design_mat,old_design,CAR_model,
                       k,  p, n_new, n_old)
    ));
}
...
// tr for inverse FI of Omega


struct invg_tr_worker : public Worker{
    // inputs
    const arma::mat & Omega_mcmc;
    const arma::mat & beta_mcmc;
    const arma::mat & mu_mcmc;
    ...
    // initialize
    invg_tr_worker(const arma::mat & Omega_mcmc,
                 const arma::mat & beta_mcmc,
                 const arma::mat & mu_mcmc,
                 const arma::mat & new_design,
                 const arma::mat & old_design,
    ...
    void operator()(std::size_t begin, std::size_t end){
        ...
        for(int i = begin ; i < end ; ++i){
            ...
            //arma::log_det(val, sign, FI_mat);
            arma::mat invFI = inv(FI_mat);
            log_dets(i) = log(sum(invFI.diag()));
        }
...
arma::vec invG_tr_para(const arma::mat & new_design,
                     const arma::mat & old_design,
                     const Rcpp::List & CAR_model,
                     int k, int p, int n_new, int n_old){
    const arma::mat & Omega_mcmc = CAR_model["Omega"];
    ...
    int n_mcmc = Omega_mcmc.n_rows;
    arma::vec log_dets(n_mcmc);
    invg_tr_worker g_det(Omega_mcmc, beta_mcmc,mu_mcmc,
                       new_design,old_design,k, p, 
                       n_new, n_old,
                       log_dets);
...
// [[Rcpp::export]]
double expected_invG_tr_para(const arma::vec & new_design,
                           const arma::mat & old_design,
                           const Rcpp::List & CAR_model,
                           int k, int p, int n_new, int n_old){
    arma::mat new_design_mat = arma::reshape(new_design,n_new,p);
    ...
    return(mean(
            invG_tr_para(new_design_mat,old_design,CAR_model,
                       k,  p, n_new, n_old)
    ));
}


// tr for pseudo-inverse FI of Omega

struct PIbeta_tr_worker : public Worker{
    // inputs
    const arma::mat & Omega_mcmc;
    const arma::mat & beta_mcmc;
    const arma::mat & mu_mcmc;
    ...
    // initialize
    PIbeta_tr_worker(const arma::mat & Omega_mcmc,
                 const arma::mat & beta_mcmc,
                 const arma::mat & mu_mcmc,
                 const arma::mat & new_design,
                 const arma::mat & old_design,
    ...
    void operator()(std::size_t begin, std::size_t end){
        ...
        for(int i = begin ; i < end ; ++i){
            ...
            //arma::log_det(val, sign, FI_mat);
            arma::mat PI_FI = arma::pinv(FI_mat);
            log_dets(i) = log(sum(PI_FI.diag()));
        }
...
// [[Rcpp::export]]
arma::vec PIbeta_tr_para(const arma::mat & new_design,
                     const arma::mat & old_design,
                     const Rcpp::List & CAR_model,
                     int k, int p, int n_new, int n_old){
    const arma::mat & Omega_mcmc = CAR_model["Omega"];
    ...
    int n_mcmc = Omega_mcmc.n_rows;
    arma::vec log_dets(n_mcmc);
    PIbeta_tr_worker beta_tr(Omega_mcmc, beta_mcmc,mu_mcmc,
                       new_design,old_design,k, p, 
                       n_new, n_old,
                       log_dets);
...
// [[Rcpp::export]]
double expected_PIbeta_tr_para(const arma::vec & new_design,
                           const arma::mat & old_design,
                           const Rcpp::List & CAR_model,
                           int k, int p, int n_new, int n_old){
    arma::mat new_design_mat = arma::reshape(new_design,n_new,p);
    ...
    return(mean(
            PIbeta_tr_para(new_design_mat,old_design,CAR_model,
                       k,  p, n_new, n_old)
    ));
}


// trace of PI of full FI

struct PIfull_tr_worker : public Worker{
    // inputs
    const arma::mat & Omega_mcmc;
    const arma::mat & beta_mcmc;
    const arma::mat & mu_mcmc;
    ...
    // initialize
    PIfull_tr_worker(const arma::mat & Omega_mcmc,
                 const arma::mat & beta_mcmc,
                 const arma::mat & mu_mcmc,
                 const arma::mat & new_design,
                 const arma::mat & old_design,
    ...
    void operator()(std::size_t begin, std::size_t end){
        ...
        for(int i = begin ; i < end ; ++i){
            ...
            //arma::log_det(val, sign, FI_mat);
            arma::mat PIFI = pinv(FI_mat);
            log_dets(i) = log(sum(PIFI.diag()));
        }
...
// [[Rcpp::export]]
arma::vec PIfull_tr_para(const arma::mat & new_design,
                     const arma::mat & old_design,
                     const Rcpp::List & CAR_model,
                     int k, int p, int n_new, int n_old){
    const arma::mat & Omega_mcmc = CAR_model["Omega"];
    ...
    int n_mcmc = Omega_mcmc.n_rows;
    arma::vec log_dets(n_mcmc);
    PIfull_tr_worker full_tr(Omega_mcmc, beta_mcmc,mu_mcmc,
                       new_design,old_design,k, p, 
                       n_new, n_old,
                       log_dets);
...
// [[Rcpp::export]]
double expected_PIfull_tr_para(const arma::vec & new_design,
                           const arma::mat & old_design,
                           const Rcpp::List & CAR_model,
                           int k, int p, int n_new, int n_old){
    arma::mat new_design_mat = arma::reshape(new_design,n_new,p);
    ...
    return(mean(
            PIfull_tr_para(new_design_mat,old_design,CAR_model,
                       k,  p, n_new, n_old)
    ));
}
