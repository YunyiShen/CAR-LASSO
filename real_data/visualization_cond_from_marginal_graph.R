...
B_cond <- multireg_beta

B_binary <- abs(B_cond) > 5e-2
Graph_binary <- abs(multireg_Graph) > 5e-2
diag(Graph_binary) <- 1

CAR <- get_CAR_MB(B_cond*B_binary,Graph_binary*multireg_Graph)
...
ggsave("./real_data/humangut2_cond_from_marg.pdf",p2,width = 8,height = 7)

ggsave("./real_data/soil2_cond_from_marg.pdf",p2,width = 8,height = 7)
