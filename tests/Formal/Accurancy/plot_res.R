...
all_loss$algo <- sub("CAR","CG",all_loss$algo)
model_list <- c("AR1","AR2","Block","Star","Circle","Dense")
all_loss$mod <- model_list[all_loss$mod]
all_loss$mod <- factor(all_loss$mod, levels = model_list)
...
beta_k30 <- ggplot(data = all_loss_k30[!is.na(all_loss_k30$logL2beta),],
  ...
  geom_boxplot(aes(fill = beta.sparsity)) + 
  facet_grid(p~mod) + 
  ylab("log Frobenius loss of beta") + 
  xlab("") + theme_bw() +
  theme(legend.position="top") + 
  theme(text = element_text(size=14), 
...
beta_k30
ggsave("./tests/Formal/Accurancy/Figs/beta_k30_Asmallprior2.pdf",beta_k30,width = 10,height = 8,unit = "in")
