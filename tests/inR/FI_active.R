...
for(i in 1:AL_round){
    ...
    res$rep[i] = min(abs(eigen(pre_info_rep[(k ) * (p+1) + 1:(.5 * (k + 1) * k),(k ) * (p+1) + 1:(.5 * (k + 1) * k)])$value))

    pre_info_rand <- pre_info_rand + 
        CAR_FI(t(rnorm(p)),Omega,beta ,mu,k,p)
    res$random[i] = min(abs(eigen(pre_info_rand[(k ) * (p+1) + 1:(.5 * (k + 1) * k),(k ) * (p+1) + 1:(.5 * (k + 1) * k)])$value)) 
    ...
    res$AL[i] = min(abs(eigen(pre_info_AL[(k ) * (p+1) + 1:(.5 * (k + 1) * k),(k ) * (p+1) + 1:(.5 * (k + 1) * k)])$value)) 
    cat(i,"\n")
